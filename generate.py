...
class Options:
    def __init__(
        self, options_filename, max_size=None, max_count=None, default_submsg_store=None
    ):
    ...
    def max_count(self, path: str) -> int:
        ...
        raise Exception(
            "Requested max_count for %s but not present in options" % (path)
        )
...
class PpbEnum:
    ...
    def __init__(
        self,
        d: descriptor_pb2.EnumDescriptorProto,
        path: str,
        known_types: KnownTypeStore,
    ):
        self.name = d.name
...
class PpbField:
    ...
    def __init__(
        ...
        self.oneof = (
            oneofs[d.oneof_index]
            if d.HasField("oneof_index") and not d.proto3_optional
            else None
        )
...
class PpbMessage:
    ...
    def __init__(
        ...
        for f in d.field:
            self.fields.append(
                PpbField(f, self.path + ".", oneofs, known_types, options)
            )
...
def get_ctype(field: PpbField) -> CType:
    if field.fieldtype.pbtype == "STRING":
        inline_struct = "struct { char str[%d]; unsigned len; }" % (
            field.fieldtype.max_encoded_size
        )
        return CType(ctype=inline_struct)
    if field.fieldtype.pbtype == "BYTES":
        inline_struct = "struct { uint8_t bytes[%d]; unsigned len; }" % (
            field.fieldtype.max_encoded_size
        )
        return CType(ctype=inline_struct)
...
def generate_c_enum_decode_fn(e: PpbEnum) -> str:
    name = field_type_name_to_cname(e.path)
    result = (
        "static bool pb_decode_enum_%s(%s *value, pb_read_fn r, void *user) {\n"
        % (name, name)
    )
...
def render_c_message_def(message: PpbMessage) -> str:
    ...
    result += (
        "bool pb_encode_%s(const struct %s *msg, pb_write_fn w, void *user);\n"
        % (struct_name, struct_name)
    )
    result += (
        "unsigned pb_encode_%s_to_buffer(uint8_t buffer[%s], const struct %s *msg);\n"
    ...
    for f in message.fields:
        result += "#define PB_TAG_%s     %dul\n" % (
            field_type_name_to_cname(f.path),
            f.tag,
        )
    result += "#define PB_MAX_SIZE_%s    %dul\n" % (
        field_type_name_to_cname(message.path),
        message.max_size,
    )
    result += "\n"
    result += "\n"
...
def generate_field_size_statements(msgptr: str, accumulator: str, f: PpbField) -> str:
    ...
    if f.is_repeated and f.fieldtype.pbtype not in ["MESSAGE", "BYTES", "STRING"]:
        ...
        if f.fieldtype.is_fixed_size:
            ...
            result += "      packed_size += %s;\n" % generate_element_size_expression(
                f, element
            )
            result += "    }\n"
...
def generate_field_encode_statement(
    f: PpbField, direct: bool = False, submsg_store_size: Optional[int] = None
) -> str:
    ...
    If submsg_store_size is not None and large enough to hold a message field, the
    ...
    use_submsg_store = (
        not direct
        and f.fieldtype.pbtype == "MESSAGE"
        and submsg_store_size is not None
        and submsg_store_size >= f.fieldtype.max_encoded_size
    )
    ...
    if f.fieldtype.pbtype not in ["MESSAGE", "BYTES", "STRING"]:
        ...
        elem = "msg->%s[idx]" % (f.name) if f.is_repeated else "msg->%s" % (f.name)
        ...
        if use_submsg_store:
            ...
            encode_stmts += (
                "unsigned elem_size = pb_encode_%s_to_buffer(submsg, &%s);\n"
        ...
        if direct:
            ...
            if f.fieldtype.pbtype == "MESSAGE":
                if use_submsg_store:
                    ...
                    encode_stmts += (
                        "if (!pb_encode_%s(&%s, w, user)) { return false; }\n"
                        % (field_type_name_to_cname(f.type_name), elem)
                    )
            elif f.fieldtype.pbtype == "STRING":
                encode_stmts += (
                    "if (!w((const uint8_t *)%s.str, %s.len, user)) { return false; }\n"
                    % (elem, elem)
                )
            elif f.fieldtype.pbtype == "BYTES":
                encode_stmts += (
                    "if (!w(%s.bytes, %s.len, user)) { return false; }\n" % (elem, elem)
                )
...
def generate_encode_stream_fn(m: PpbMessage) -> str:
    ...
    for f in m.fields:
        result += "  if (%s) {\n" % (generate_presence_expression("msg", f))
        result += indent(
            generate_field_encode_statement(
                f, direct=False, submsg_store_size=m.submsg_store
            ),
            "    ",
        )
        result += "  }\n"
...
def generate_field_decode_statements(f: PpbField) -> str:
    ...
    if f.is_repeated or f.fieldtype.pbtype in ["STRING", "MESSAGE", "BYTES"]:
        ...
        if f.fieldtype.pbtype == "STRING":
            ...
            result += (
                "  if (!r((uint8_t *)%s.str, length, user)) { return false; }\n"
                % (fieldname)
            )
            result += "  %s.len = length;\n" % (fieldname)
...
def generate_c_source(files: List[PpbFile], source_filename: str, header_filename: str):
    ...
    with open(source_filename, "w") as wf:
...
def main():
    ...
    parser.add_argument(
        "--options",
        action="store",
        default="viapb.options",
        help="Path to options file for setting field sizes/lengths",
    )
    parser.add_argument(
        "-f",
        "--file",
        action="store",
        required=True,
        help="Path to a compiled FileDescriptorSet for the protocol files to generate from",
    )
    parser.add_argument(
        "-c",
        "--output-source",
        action="store",
        default="viapb-generated.c",
        help="path to output generated C source file",
    )
    parser.add_argument(
        "-H",
        "--output-header",
        action="store",
        default="viapb-generated.h",
        help="path to output generated C header file",
    )
    parser.add_argument(
        "--default-max-size",
        action="store",
        type=int,
        help="default max size for string/bytes fields when otherwise not specified",
    )
    parser.add_argument(
        "--default-max-count",
        action="store",
        type=int,
        help="default max count of repeated fields when otherwise unspecified",
    )
    parser.add_argument(
        "--default-submsg-store",
        action="store",
        type=int,
        help="default max count of repeated fields when otherwise unspecified",
    )
    ...
    options = Options(
        args.options,
        args.default_max_size,
        args.default_max_count,
        args.default_submsg_store,
    )
    ...
    generate_c_source(
        parsed_files,
        source_filename=args.output_source,
        header_filename=args.output_header,
    )
